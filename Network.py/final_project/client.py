...
def play_question(conn):
	...
	if cmd == 'ERROR_OUT_OF_QUASTIONS':
		...
		return 'ERR'
	msg_num = "".join(message.split('#')[0])
	msg_question = " ".join("".join(message.split('?')[0]).split('#')[1:])
	msg_options = "".join(message.split('?')[1])
	print(f'(Q) Your question is: {msg_question}')
	for num, elem in enumerate(msg_options.split('#'), 1):
		if num == 1:
			print(f'\n> is it:\n{num}) {elem}')
			continue
		print(f'{num}) {elem}')
	...
	(cmd, message) = build_send_recv_parse(conn, chatlib.PROTOCOL_CLIENT["user_answer"], msg_num+'/'+user_answer)
...
def login(conn):
	...
	while True:
		...
		if command == chatlib.PROTOCOL_SERVER["login_ok_msg"]:
			...
			print(f'Login unsucssesful. Server response: {command} => {msg.replace("#", " ")}')
	return
...
Please enter your choice: 
		...
		match user_choice:
			case "score":
				(command, msg) = build_send_recv_parse(client_socket_object, chatlib.PROTOCOL_CLIENT[user_choice], Username)
				if command == 'ERROR':
					print(f'Choice ERROR. Server response: {command} => {msg.replace("#", " ")}')
					continue
